...
public class ValidationPanierServlet extends HttpServlet 
{
		...
		if (panier.size() > 0)
		{
			...
			try 
			{
				...
				if (i == panier.size() && possible)
				{
					...
					{
						Panier.clearUserPanier(request, req, res);
						request.commit();
					}
					request.close();
				}
	...
	public static Vector<Place> validerLignePanier(Transaction request, ContenuPanier reserv) throws IOException, ConnectionException, RequestException
	{
	...
	public static boolean validationOk(Transaction request, ServletOutputStream out, Panier panier, Vector<Vector<Place>> placesRes) throws IOException, ConnectionException, RequestException
	{
		int nbPlaces;
		if (placesRes.size() == 0)
		{
		...
		for(int i = 0 ; i < panier.size() ; i++)
		{
			...
			out.println(panier.getContenu(i).getCategorie().getNom() 
